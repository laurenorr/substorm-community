%<Plot_3_Modularity_histogram.m Plots figure 3 from Orr.et.al 2021.>
...
load('Substom_list_extreme_quiet.mat')
load('Substom_list_decent_coverage.mat')

Mod_flag='normed';
LIM=0.55;
P=modularity_hist_overlay_med(normalized_times2,networki_v4_w128_t5.eb,intersect([Extreme_quiet],coverage123),'all_123_extreme_quiet_col_w128_t5EB',LIM,Mod_flag);

function[P]=modularity_hist_overlay_med(NORM_TM,NETWORK,subs,NAME,probQ,Mod_flag)
close all
clmap =[0         0         0
    0.0345         0    0.0345
    0.0690         0    0.0690
...
sp7=[0.1,0.7,0.8,0.25;
    ...
    ac.Box='on';
    set(gca,'fontsize',22)
    P.A=[[-20:50];mod_counts./sum(mod_counts,1)];
    
    subplot('Position',sp7(2,:)); hold on;
    for i=subs
        plot(-20:1:50,MOD_Norm(i,:),'color',[0.7,0.73,0.7,0.15],'linewidth',4.5);
        hold on;
    end
    plot(-20:1:50,median(MOD_Norm(subs,:),'omitnan'),'color',[0,0,0],'linewidth',5)
    plot(-20:1:50,quantile(MOD_Norm(subs,:),0.25),'color',[0.4,0.4,0.4],'linewidth',4)
    plot(-20:1:50,quantile(MOD_Norm(subs,:),0.75),'color',[0.4,0.4,0.4],'linewidth',4)
    P.B=MOD_Norm(subs,:);
    P.Bo=[median(MOD_Norm(subs,:),'omitnan');quantile(MOD_Norm(subs,:),0.25);quantile(MOD_Norm(subs,:),0.75)];
    ylim([0,upperlimm])
    ylimitz=[0,upperlimm];
    a1=plot([0,0],ylimitz,'--','Color',[0.1000,0.6000,0.2000],'linewidth',4);
    plot([10,10],ylimitz,'--','Color',[0.5,0.5,0.5],'linewidth',4);
...
for sp=1:7
    ...
    P.C(sp,:)=histcounts(MOD_Norm(subs,TM),[0:0.05:1],'Normalization','probability');
    ...
    P.Co(sp,1)=median(median(MOD_Norm(subs,TM),'omitnan'),'omitnan');
    ac=gca;
    ac.Box='on';
    xlim([0,upperlimm])
    xticks([0:0.5:1])
    ...
    annotation('textbox',[0.87,0.95,0,0],'String','a','FitBoxToText','on','color',[1,1,1],'FontSize',36,'FontWeight','bold','EdgeColor','none')
    annotation('textbox',[0.87,0.67,0,0],'String','b','FitBoxToText','on','FontSize',36,'FontWeight','bold','EdgeColor','none')
    annotation('textbox',[0.87,0.31,0,0],'String','c','FitBoxToText','on','FontSize',36,'FontWeight','bold','EdgeColor','none')
    
    if Mod_flag=='unnorm'
            print(['pngs/Mod_ophist_unnormd_MED_ge5',NAME],'-dpng');
    else
            print(['pngs/Mod_ophist_normd_MED_ge5',NAME],'-dpng');
    end
